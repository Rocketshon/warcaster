    ...
    return (
      <div className="min-h-screen bg-[var(--bg-primary)] flex flex-col p-6 relative overflow-hidden">
        <div className="relative z-10 w-full max-w-md mx-auto">
          ...
          <p className="text-[var(--text-secondary)] text-center mt-12">Unit not found.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[var(--bg-primary)] flex flex-col p-6 relative overflow-hidden pb-24">
      <div className="relative z-10 w-full max-w-md mx-auto">
        ...
        <div className="mb-6">
          <div className="flex items-start justify-between gap-3 mb-3">
            <div className="flex-1">
              ...
              <h1 className="text-2xl font-bold text-[var(--text-primary)] tracking-wider mb-1">
                {unit.custom_name}
              </h1>
              <p className="text-[var(--text-secondary)] text-sm italic mb-2">{unit.datasheet_name}</p>
            </div>
            <div className="text-right">
              ...
              <button
                ...
                className="text-xs text-[var(--accent-gold)]/70 hover:text-[var(--accent-gold)] transition-colors flex items-center gap-1 mt-1"
              >
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-3 gap-2 mb-2">
              {Object.entries(stats).map(([key, value]) => (
                <div
                  key={key}
                  className="rounded-sm border border-[var(--border-color)] bg-[var(--bg-card)] p-3"
                >
            ...
            {invulnSave && (
              <div className="rounded-sm border border-[var(--border-color)] bg-[var(--bg-card)] p-3">
                <div className="text-center">
        ...
        {datasheet?.wargear_options && datasheet.wargear_options.length > 0 && (
          <div className="mb-6">
            <h2 className="text-sm font-semibold text-[var(--text-tertiary)] uppercase tracking-wider mb-3">Wargear Options</h2>
            <div className="rounded-sm border border-[var(--border-color)] bg-[var(--bg-card)] p-3">
        ...
        {datasheet?.wargear_abilities && datasheet.wargear_abilities.length > 0 && (
          <div className="mb-6">
            <h2 className="text-sm font-semibold text-[var(--text-tertiary)] uppercase tracking-wider mb-3">Wargear Abilities</h2>
            <div className="space-y-2">
              {datasheet.wargear_abilities.map((ability, idx: number) => {
                ...
                return (
                  <div key={idx} className="rounded-sm border border-[var(--border-color)] bg-[var(--bg-card)] p-3">
                    <h3 className="text-xs font-bold text-[var(--accent-gold)] mb-1">{ability[0]}</h3>
        ...
        {abilities.length > 0 && (
          <div className="mb-6">
            <button
              ...
              className="w-full flex items-center justify-between text-sm font-semibold text-[var(--text-tertiary)] uppercase tracking-wider mb-3 hover:text-[var(--accent-gold)] transition-colors"
            >
              <span>Abilities ({abilities.length})</span>
              {showAbilities ? (
                <ChevronUp className="w-5 h-5 text-[var(--accent-gold)]" />
              ) : (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-1.5">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <span className="text-[10px] text-[var(--text-secondary)] uppercase">{strat.type}</span>
                        <span className="text-xs font-bold text-amber-600 font-mono bg-amber-500/10 px-1.5 py-0.5 rounded">
                          {strat.cp}
                        </span>
                      </div>
                    </div>
                    <p className="text-[10px] text-[var(--text-secondary)] mb-1">{strat.detachment}</p>
                    <div className="space-y-1 text-xs text-[var(--text-secondary)] leading-relaxed">
                      <p><span className="text-[var(--text-tertiary)] font-medium">When:</span> {strat.when}</p>
                      <p><span className="text-[var(--text-tertiary)] font-medium">Target:</span> {strat.target}</p>
                      <p><span className="text-[var(--text-tertiary)] font-medium">Effect:</span> {strat.effect}</p>
                      {strat.restrictions && (
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="text-xs text-[var(--accent-gold)]/70 hover:text-[var(--accent-gold)] transition-colors flex items-center gap-1"
                  >
              ...
              <div className="space-y-2">
                ...
                {unit.battle_honours.length === 0 && (
                  <p className="text-xs text-[var(--text-secondary)] italic">No battle honours yet.</p>
                )}
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="space-y-2">
                ...
                {unit.battle_scars.length === 0 && (
                  <p className="text-xs text-[var(--text-secondary)] italic">No battle scars yet.</p>
                )}
              </div>
            </div>
      ...
      {isEditing && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-6" role="dialog" aria-modal="true">
          <div className="w-full max-w-sm rounded-sm border border-[var(--border-color)] bg-[var(--bg-primary)] p-6">
            <div>
              <h3 className="text-lg font-bold text-[var(--text-primary)] mb-4">Edit Unit</h3>

              <div className="space-y-4">
                <div>
                  <label className="block text-xs text-[var(--text-secondary)] uppercase tracking-wider mb-1">Custom Name</label>
                  <input
                    type="text"
                    value={editName}
                    onChange={(e) => setEditName(e.target.value)}
                    className="w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[var(--text-primary)] text-sm focus:border-[var(--accent-gold)]/40 focus:outline-none focus:ring-2 focus:ring-[var(--accent-gold)]/20"
                  />
                </div>

                <div>
                  <label className="block text-xs text-[var(--text-secondary)] uppercase tracking-wider mb-1">Points Cost</label>
                  <input
                    type="number"
                    inputMode="numeric"
                    value={editPoints}
                ...
                <div>
                  <label className="block text-xs text-[var(--text-secondary)] uppercase tracking-wider mb-1">Notes</label>
                  <textarea
                    value={editNotes}
                    onChange={(e) => setEditNotes(e.target.value)}
                    rows={3}
                    placeholder="Battle notes, lore, etc."
                    className="w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg px-3 py-2 text-[var(--text-primary)] text-sm focus:border-[var(--accent-gold)]/40 focus:outline-none focus:ring-2 focus:ring-[var(--accent-gold)]/20 resize-none"
                  />
                </div>
              </div>
              ...
              <div className="flex gap-3 mt-6">
                <button
                  ...
                  className="flex-1 px-4 py-2 rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] text-[var(--text-tertiary)] font-semibold hover:border-[var(--accent-gold)] transition-all"
                >
                  Cancel
                </button>
                <button
      ...
      {showEnhancementPicker && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-6" role="dialog" aria-modal="true">
          <div className="w-full max-w-sm rounded-sm border border-[var(--border-color)] bg-[var(--bg-primary)] p-6 max-h-[80vh] overflow-y-auto">
            <div>
              ...
              <div className="space-y-3">
                {filteredEnhancements.map((enh, idx) => (
                  <button
                    ...
                    className="w-full text-left rounded-sm border border-[var(--border-color)] bg-[var(--bg-card)] p-3 hover:border-[var(--accent-gold)] transition-all"
                  >
                    ...
                    <p className="text-[10px] text-[var(--text-secondary)] mb-1">{enh.detachment}</p>
              ...
              <button
                ...
                className="w-full mt-4 px-4 py-2 rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] text-[var(--text-tertiary)] font-semibold hover:border-[var(--accent-gold)] transition-all"
              >
                Cancel
              </button>
            </div>
      ...
      {showSpendXP && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-6" role="dialog" aria-modal="true">
          <div className="w-full max-w-sm rounded-sm border border-[var(--border-color)] bg-[var(--bg-primary)] p-6">
            <div>
              <div className="flex justify-center mb-4">
                <Star className="w-12 h-12 text-[var(--accent-gold)]" strokeWidth={1.5} />
              </div>
              <h3 className="text-lg font-bold text-[var(--text-primary)] text-center mb-2">
                Award Experience
              </h3>
              <p className="text-sm text-[var(--text-secondary)] text-center mb-4">
                Current XP: <span className="text-[var(--accent-gold)] font-mono">{unit.experience_points}</span>
              </p>
              ...
              <div className="flex items-center justify-center gap-4 mb-6">
                ...
                <div className="text-3xl font-bold text-[var(--accent-gold)] font-mono w-16 text-center">
                  {xpAmount}
                </div>
                <button
