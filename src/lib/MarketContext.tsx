...
interface MarketState {
  ...
  createListing: (data: CreateListingInput, photos: File[]) => Promise<string>;
...
export function MarketProvider({ children }: { children: ReactNode }) {
  ...
  const createListing = useCallback(async (input: CreateListingInput, photos: File[]): Promise<string> => {
    ...
    // Upload photos (resized to max 1200px, JPEG 0.8 quality)
    if (photos.length > 0) {
      const paths: string[] = [];
      for (let i = 0; i < photos.length; i++) {
        const resized = await resizeImage(photos[i]);
